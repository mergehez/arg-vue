    ...
    <CenteredModal
        ...
        <div class="flex flex-col" style="min-width: min(400px, 70vw);">
            ...
            <footer class="flex justify-end gap-3 px-5 py-3">
                <button :class="twMerge('btn py-0.5 rounded font-normal', state.classCancel)" @click="cancel">
                    {{ state.textCancel }}
                </button>
                <button
                    :class="twMerge('btn py-0.5 rounded font-normal', state.classConfirm, submitEnabled ? '' : '!opacity-50')"
                    @click="confirm"

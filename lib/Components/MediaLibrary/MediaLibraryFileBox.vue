...
import {openConfirmationDialog, showNotificationOpts} from "../Overlays";
...
const emit = defineEmits<{
    selected: [IM.FileInfo],
    deleted: [IM.FileInfo],
}>();

const isSelection = computed(() => props.state.selection == props.f);
        ...
        onConfirm: () => {
            props.state.apiPost('delete-files', {
                ...
                showNotificationOpts({
                    message: `The file has been deleted: ${file.name}`,
                    type: 'success',
                    duration: 5000,
                })
                emit('deleted', file);
